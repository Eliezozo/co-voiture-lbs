-- LBS Carpooling PWA - FULL SINGLE SQL SCRIPT
-- This script is the single source of truth.
-- It resets and recreates the schema for this project.
...
DROP TRIGGER IF EXISTS set_trip_price_before_write ON trips;
...
BEGIN
  ...
  SELECT t.driver_id, t.price
    INTO v_driver_id, v_price
  FROM bookings b
...
DROP POLICY IF EXISTS "trips_insert_by_driver" ON trips;
...
-- Backfill missing profile rows from auth.users (useful if users already exist)
INSERT INTO public.profiles (id, email, full_name, role)
SELECT
  u.id,
  u.email,
